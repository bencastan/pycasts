...
def save_new_episodes(feed):
    ...
    databse. If not found, then anew `Episode` is added to the database.
...
def fetch_abc_science_show_episodes():
    """
    Fetches new episodes from RSS for the ABC Science Show Podcast.
    """
    _feed = feedparser.parse("https://www.abc.net.au/radionational/programs/scienceshow/feed/2885480/podcast.xml")
    save_new_episodes(_feed)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduler.add_job(
            fetch_abc_science_show_episodes,
            trigger="interval",
            minutes=1440, # 1 Day
            id="ABC Science Show",
            max_instances=1,
        ...
        scheduler.add_job(
            ...
            ), # Midnight on MOnday, before start of the nect work week.
            id="Delete Old Job Executions",
            max_instances=1,
            replace_existing=True,
        )
        logger.info("Added weekly job: Deleet Old Job executions.")
